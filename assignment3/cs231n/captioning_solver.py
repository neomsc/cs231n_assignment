...
class CaptioningSolver(object):
    ...
    def _step(self):
        ...
        loss, grads = self.model.loss(features, captions,self.weight_decay)
    ...
    def BLEU_score(self,gt_caption, sample_caption):
        ...
        hypothesis = [x for x in sample_caption.split(' ')
                      if ('<END>' not in x and '<START>' not in x and '<UNK>' not in x)]
        BLEUscore = nltk.translate.bleu_score.sentence_bleu([reference], hypothesis, weights=[1],smoothing_function = nltk.translate.bleu_score.SmoothingFunction().method1)
        return BLEUscore
    ...
    def evaluate_model(self):
        ...
        for split in ['train', 'val']:
            minibatch = sample_coco_minibatch(self.data, split=split, batch_size=50)
